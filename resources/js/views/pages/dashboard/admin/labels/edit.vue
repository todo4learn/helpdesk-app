            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 px-5">
                <div class="md:flex md:items-center md:justify-between">
                    <div class="flex-1 min-w-0">
                        <h1 class="py-0.5 text-2xl font-semibold text-gray-900">{{ $t('Edit label') }}</h1>
                    </div>
                ...
                <div class="mt-6 shadow sm:rounded-lg">
                    ...
                    <div class="bg-white md:grid md:grid-cols-3 md:gap-6 px-4 py-5 sm:p-6">
                        <div class="md:col-span-1">
                            <h3 class="text-lg font-medium leading-6 text-gray-900">{{ $t('Label details') }}</h3>
                            <p class="mt-1 text-sm leading-5 text-gray-500">
                                {{ $t('Label details and customization') }}.
                            </p>
                        </div>
                        <div class="mt-5 md:mt-0 md:col-span-2">
                            <div class="grid grid-cols-3 gap-6">
                                ...
                                <div class="col-span-3">
                                    <input-color id="color" v-model="label.color" :label="$t('Color')" :placeholder="$t('Pick a color')" required></input-color>
                                </div>
                            </div>
                        </div>
                    </div>
                                ...
                                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                    ...
                                    <div class="mt-2">
                                        <p class="text-sm leading-5 text-gray-500">
                                            {{ $t('Are you sure you want to delete the label?') }}
...
export default {
    ...
    methods: {
        saveLabel() {
            const self = this;
            self.loading = true;
            axios.post('api/dashboard/admin/labels/' + self.$route.params.id, self.label).then(function () {
                self.loading = false;
                self.$notify({
        ...
        getLabel() {
            const self = this;
            self.loading = true;
            axios.get('api/dashboard/admin/labels/' + self.$route.params.id).then(function (response) {
        ...
        deleteLabel() {
            const self = this;
            axios.delete('api/dashboard/admin/labels/' + self.$route.params.id).then(function () {
                self.$notify({
                    title: self.$i18n.t('Success').toString(),
                    text: self.$i18n.t('Data deleted successfully').toString(),
                    type: 'success'
                });
                self.$router.push('/dashboard/admin/labels');
            }).catch(function () {
