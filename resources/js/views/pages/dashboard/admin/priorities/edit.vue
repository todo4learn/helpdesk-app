            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 px-5">
                <div class="md:flex md:items-center md:justify-between">
                    <div class="flex-1 min-w-0">
                        <h1 class="py-0.5 text-2xl font-semibold text-gray-900">{{ $t('Edit priority') }}</h1>
                    </div>
                </div>
            </div>
                ...
                <div class="mt-6 shadow sm:rounded-lg">
                    ...
                    <div class="bg-white md:grid md:grid-cols-3 md:gap-6 px-4 py-5 sm:p-6">
                        <div class="md:col-span-1">
                            <h3 class="text-lg font-medium leading-6 text-gray-900">{{ $t('Priority details') }}</h3>
                            <p class="mt-1 text-sm leading-5 text-gray-500">
                                {{ $t('Priority details and customization') }}.
                            </p>
                        </div>
                        <div class="mt-5 md:mt-0 md:col-span-2">
                            <div class="grid grid-cols-3 gap-6">
                                ...
                                <div class="col-span-3">
                                    <label class="block text-sm font-medium leading-5 text-gray-700" for="value">{{ $t('Value') }}</label>
                                    <div class="mt-1 relative rounded-md shadow-sm">
                                        <input
                                            id="value"
...
export default {
    ...
    metaInfo() {
        return {
            title: this.$i18n.t('Edit priority')
        }
    },
    data() {
        return {
    ...
    methods: {
        ...
        getPriority() {
            const self = this;
            self.loading = true;
            axios.get('api/dashboard/admin/priorities/' + self.$route.params.id).then(function (response) {
                self.priority = response.data;
                self.loading = false;
            }).catch(function () {
                self.loading = false;
            });
